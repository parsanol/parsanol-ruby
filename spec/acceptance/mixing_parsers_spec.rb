...
describe "Mixing parsers using alternate" do
  class MixedParsersParser
    ...
    rule(:parser1) do
      str("a") >> match('\d').repeat(1).as(:number) >> (str(":") >> match('\d').repeat(
        4, 4
      ).as(:year)).maybe
    end
  ...
  class TransformIdentifiers < Parsanol::Transform
  ...
  describe MixedParsersParser do
    ...
    let(:should_match_both_parsers) do
      "(#{should_match_parser1}, #{should_match_parser2})"
    end
    ...
    it "fails with alternating parsers" do
      ...
      expect(subject.failing_rule_example).not_to parse(
        "(#{should_match_parser2})", trace: true
      )
      expect(subject.failing_rule_example).not_to parse(
        should_match_both_parsers, trace: true
      )
    end
