...
RSpec.describe "Parslet API Compatibility" do
  ...
  describe "Transform class" do
    let(:transform_class) do
      Class.new(Parsanol::Parslet::Transform) do
        ...
        rule(num: simple(:n), word: simple(:w)) do
          { number: Integer(n), word: w.to_s.upcase }
        end
      end
    end
  ...
  describe "Native parser compatibility",
           if: defined?(Parsanol::Native) && Parsanol::Native.available? do
    describe "sequence flattening" do
      ...
      it "distinguishes wrapper pattern from repetition pattern" do
        ...
        repetition_result = Parsanol::Native::Parser.parse(repetition_parser,
                                                           "ab")
