...
describe Parsanol::ZeroCopy do
  ...
  let(:parser_class) do
    Class.new(Parsanol::Parser) do
      ...
      rule(:expression) do
        (number.as(:left) >> add_op.as(:op) >> expression.as(:right)).as(:binop) | number
      end
      root(:expression)
    end
  end
  ...
  describe "#parse" do
    ...
    context "when output_types is not defined" do
      before do
        allow(Parsanol::Native).to receive_messages(available?: true,
                                                    serialize_grammar: "{}")
      end
      ...
      it "raises ArgumentError" do
        ...
        expect do
          fresh_parser.parse("a")
        end.to raise_error(ArgumentError,
                           /ZeroCopy mode requires output_types/)
      end
    end
  end
  ...
  describe "#parse_with_types" do
    context "when native extension is not available" do
      ...
      it "raises LoadError" do
        expect do
          parser.parse_with_types("42", {})
        end.to raise_error(LoadError, /ZeroCopy mode requires native extension/)
      end
    end
  end
