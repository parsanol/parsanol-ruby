...
RSpec.describe Parsanol::Parallel do
  ...
  describe ".available_cores" do
    it "returns an integer" do
      result = described_class.available_cores
  ...
  describe ".optimal_threads" do
    it "returns minimum of cores and input count" do
      cores = described_class.available_cores

      # Small input count
      expect(described_class.optimal_threads(2)).to eq([2, cores].min)

      # Large input count
      expect(described_class.optimal_threads(1000)).to eq(cores)
    end
  end
  ...
  describe ".parse_batch" do
    context "when native extension is not available" do
      ...
      it "raises LoadError" do
        expect do
          described_class.parse_batch("{}", ["input"])
