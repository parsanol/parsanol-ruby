...
describe Parsanol::Pattern do
  ...
  RSpec::Matchers.define :match_with_bind do |pattern, exp_bindings|
    ...
    match do |tree|
      bindings = described_class.new(pattern).match(tree)
  ...
  describe "<- #match" do
    context "injecting bindings" do
      ...
      it "does not modify the original bindings hash" do
        h = {}
        b = pattern.match("a", h)
      ...
      it "returns nil when no match succeeds" do
      ...
      context "when matching simple(:x) against 'a'" do
        ...
        before { bindings.should_not be_nil }

        it "returns the injected bindings" do
          bindings[:foo].should == :bar
        end

        it "returns the new bindings" do
          bindings[:x].should == "a"
        end
      end
    end
    ...
    context "simple hash {:a => 'b'}" do
      ...
      it "does not match {:a => simple(:x), :b => simple(:y)}" do
        exp.should_not detect(a: simple(:x), b: simple(:y))
      end

      it "matches {:a => simple(:x)}, binding 'x' to the first argument" do
        exp.should match_with_bind({ a: simple(:x) }, x: "b")
      end

      it "matches {:a => 'b'} with no binds" do
    ...
    context "a more complex hash {:a => {:b => 'c'}}" do
      ...
      it "matches wholly with {:a => {:b => simple(:x)}}" do
        exp.should match_with_bind({ a: { b: simple(:x) } }, x: "c")
      end

      it "matches wholly with {:a => subtree(:t)}" do
        with_match_locals(a: subtree(:t)) do |dict|
          dict[:t].should == { b: "c" }
        end
      end

      it "does not bind subtrees to variables in {:a => simple(:x)}" do
    ...
    context "a more complex hash {:a => 'a', :b => 'b'}" do
      ...
      it "does not match partially" do
        described_class.new(a: simple(:x)).match(exp).should be_nil
      end

      it "matches completely" do
    ...
    context "an array of 'a', 'b', 'c'" do
      ...
      it "matches all elements at once" do
    ...
    context "{:a => 'a', :b => 'b'}" do
      ...
      it "matches both elements simple(:x), simple(:y)" do
        exp.should match_with_bind(
      ...
      it "does not match a constrained match (simple(:x) != simple(:y))" do
    ...
    context "{:a => 'a', :b => 'a'}" do
      ...
      it "matches constrained pattern" do
        exp.should match_with_bind(
    ...
    context "{:sub1 => {:a => 'a'}, :sub2 => {:a => 'a'}}" do
      ...
      it "verifies constraints over several subtrees" do
        exp.should match_with_bind({
                                     sub1: { a: simple(:x) },
                                     sub2: { a: simple(:x) },
                                   }, x: "a")
      ...
      it "returns both bind variables simple(:x), simple(:y)" do
        exp.should match_with_bind({
                                     sub1: { a: simple(:x) },
                                     sub2: { a: simple(:y) },
                                   }, x: "a", y: "a")
    ...
    context "[{:a => 'x'}, {:a => 'y'}]" do
      ...
      it "does not match sequence(:x) (as a whole)" do
    ...
    context "['x', 'y', 'z']" do
      ...
      it "matches [simple(:x), simple(:y), simple(:z)]" do
      ...
      it "matches %w(x y z)" do
        exp.should match_with_bind(%w[x y z], {})
      end

      it "does not match [simple(:x), simple(:y), simple(:x)]" do
        exp.should_not detect([simple(:x), simple(:y), simple(:x)])
      end

      it "does not match [simple(:x), simple(:y)]" do
        exp.should_not detect([simple(:x), simple(:y), simple(:x)])
      end

      it "matches sequence(:x) (as array)" do
    ...
    context "{:a => [1,2,3]}" do
      ...
      it "matches :a => sequence(:x) (binding x to the whole array)" do
