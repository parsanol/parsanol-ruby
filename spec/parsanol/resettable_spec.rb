...
describe Parsanol::Resettable do
  describe "pooled classes" do
    it "Slice includes Resettable" do
      expect(Parsanol::Slice.ancestors).to include(described_class)
    end

    it "Position includes Resettable" do
      expect(Parsanol::Position.ancestors).to include(described_class)
    end

    it "Buffer includes Resettable" do
      expect(Parsanol::Buffer.ancestors).to include(described_class)
    end

    it "StringView includes Resettable" do
      expect(Parsanol::StringView.ancestors).to include(described_class)
    end
  end
  ...
  describe "explicit contract" do
    ...
    it "avoids duck-typing with respond_to? checks" do
      ...
      expect(slice.is_a?(described_class)).to be true

      position = Parsanol::Position.new("test", 0)
      expect(position.is_a?(described_class)).to be true

      buffer = Parsanol::Buffer.new
      expect(buffer.is_a?(described_class)).to be true

      view = Parsanol::StringView.new("test")
      expect(view.is_a?(described_class)).to be true
    end
  end
end
