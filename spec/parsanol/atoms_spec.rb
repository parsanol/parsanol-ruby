...
describe Parsanol do
  ...
  describe "eof behaviour" do
    ...
    context "when the pattern doesn't match the input" do
      ...
      it "raises Parsanol::ParseFailed" do
        ...
        exception.should be_a(Parsanol::ParseFailed)
      end
  ...
  describe "<- #as(name)" do
    ...
    context "str('a').as(:a).repeat" do
      it "returns an array of subtrees" do
        expect(strip_positions(str("a").as(:a).repeat.parse("aa"))).to eq([
                                                                            { a: "a" }, { a: "a" }
                                                                          ])
      end
    end
  ...
  describe "<- #flatten(val)" do
    ...
    [
      ...
      [[:sequence, [:repetition, { a: "a" }], { a: "a" }],
       [{ a: "a" }, { a: "a" }]],
      [[:sequence, { a: "a" }, [:repetition, { a: "a" }]],
       [{ a: "a" }, { a: "a" }]],
      [[:sequence, [:repetition, { a: "a" }], [:repetition, { a: "a" }]],
       [{ a: "a" }, { a: "a" }]],
      # Repetition
