...
class Parsanol::Atoms::Base
  ...
  UNCONSUMED_INPUT_MSG = "Don't know what to do with ".freeze
  ...
  def apply(input, context, consume_all = false)
    ...
    if consume_all && input.chars_left.positive?
      return unconsumed_error(input, context, position_before)
    end
  ...
  def report_detailed_error(input, consume_all, reporter, initial_error)
    input.bytepos = 0
    ...
    if success
      raise "Invariant violation: parse succeeded during error reporting"
    end
