...
class Parsanol::Atoms::Repetition < Parsanol::Atoms::Base
  ...
  def initialize(parser, min_count, max_count, tag = :repetition)
    ...
    @min_error = "Expected at least #{min_count} of #{parser.inspect}".freeze
    @extra_error = "Extra input after last repetition".freeze
  end
  ...
  def try(source, context, consume_all)
    ...
    return try_maybe(source, context, consume_all) if @min == 0 && @max == 1
  ...
  def compute_first_set
    ...
    first.add(Parsanol::FirstSet::EPSILON) if @min == 0
    first
  end
  ...
  def try_maybe(source, context, consume_all)
